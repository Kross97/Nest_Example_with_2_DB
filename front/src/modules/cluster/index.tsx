...
export const ClusterBlock = () => {
  ...
  const getCurrentPorts = () => {
    FetchAgent.getRequest({ url: '/childProcess_cluster/cluster_ports' }).then((results: IPortsData) => {
  ...
  const enableClusterHandle = () => FetchAgent.getRequest({ url: '/childProcess_cluster/cluster_enable' }).then(() => {
    getCurrentPorts();
  });
  ...
  return <div className={cn.clusterBlock}>
    ...
    <div>
      <button disabled={currentPorts.length === 1} onClick={setNewPortServer}>Выбрать новый порт сервера</button>
      <button disabled={currentPorts.length !== 1} onClick={enableClusterHandle}>Включить кластеризацию</button>
      <button disabled={currentPorts.length === 1} onClick={disableClusterHandler}>Отключить кластеризацию</button>
    </div>
  </div>;
};