  ...
  return (
    <div className={cn.photosBlock}>
      ...
      <div className={cn.column}>
        ...
        <input onChange={changeMany} multiple type="file" />
      </div>
      ...
      <div className={cn.listFiles}>
        {files.map((f: IFile) => {
          return (
            ...
            >
              <b>{f.name}</b>
              <button style={{ alignSelf: "flex-start" }} onClick={() => downloadFile(f)}>
                загрузка_способ_1
              </button>
              <button style={{ alignSelf: "flex-start" }} onClick={() => downloadFile2(f)}>
                загрузка_способ_2
              </button>
            </div>
          );
